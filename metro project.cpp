...
void findMinimumSpanningTree(const string& src) {
        ...
        for (int i = 0; i < numVertex() - 1; ++i) {
            string currVertex = getMinimumDistanceVertex(dist, visited);
            visited[currVertex] = true;

            const Vertex& vtx = vtces[currVertex];
            for (const auto& nbr : vtx.nbrs) {
                const string& nbrVertex = nbr.first;
        ...
        cout << "Minimum path and and total distance required to cover all stations:" << endl;
...
string getMinimumDistanceVertex(const unordered_map<string, int>& dist, const unordered_map<string, bool>& visited) {
        int minDist = INT_MAX;
        string minVertex;
...
int main() {
    ...
    //g.addEdge("Karol Bagh","Dwarka",10);
